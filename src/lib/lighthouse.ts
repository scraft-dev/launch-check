...
export function buildLighthouseAudit(
  scanResult: ScanResponse,
): LighthouseAudit {
  const browserMetrics = (
    scanResult as ScanResponse & {
      lighthouseMetrics?: LighthouseMetrics;
    }
  ).lighthouseMetrics;

  const performance =
    browserMetrics?.performance ??
    clamp(
      scanResult.httpStatus >= 200 && scanResult.httpStatus < 400 ? 78 : 54,
    );
  const accessibility =
    browserMetrics?.accessibility ??
    (scanResult.httpStatus >= 200 && scanResult.httpStatus < 400 ? 88 : 74);
  const bestPractices =
    browserMetrics?.bestPractices ??
    (scanResult.pageErrors.length === 0 ? 82 : 64);
  ...
  if (scanResult.consoleErrors.length > 0) {
    opportunities.push({
      ...
      detail:
        "Address console errors to improve stability and developer experience.",
    });
  }
