...
export default function HistoryDetailPage({
  ...
  useEffect(() => {
    async function loadScan() {
      ...
      const selectedScan =
        history.find((item) => item.id === resolvedParams.id) ?? null;
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-10 text-slate-900">
      <div className="mx-auto max-w-3xl rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <div className="mt-6 space-y-3 rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">
          ...
          <p>
            <span className="font-semibold text-slate-900">Final URL:</span>{" "}
            {scan.finalUrl}
          </p>
          <p>
            <span className="font-semibold text-slate-900">HTTP Status:</span>{" "}
            {scan.httpStatus}
          </p>
          <p>
            <span className="font-semibold text-slate-900">Load Time:</span>{" "}
            {scan.loadTime}ms
          </p>
          <p>
